// -----------------------------------------------------------------------
//  <copyright file="DataGridViewPrinter.cs" company="Curtis Kaler">
...
namespace ServiceSentry.UnitTests.Common.Printing
{
    public class DataGridViewPrinter
    {
            ...
            if (_pageNumber == 0) // Just calculate once
            {
                ...
                for (var i = 0; i < _theDataGridView.Columns.Count; i++)
                {
                    ...
                    if (tmpFont == null)
                        ...
                        tmpFont = _theDataGridView.DefaultCellStyle.Font;
                    ...
                    {
                        tmpFont = _theDataGridView.Rows[j].DefaultCellStyle.Font;
                        if (tmpFont == null)
                ...
                var mTempPrintArea = (float) _pageWidth - (float) _leftMargin - (float) _rightMargin;
        ...
        private void DrawHeader(Graphics g)
        {
            ...
            {
                _pageNumber++;
                var pageString = "Page " + _pageNumber.ToString(CultureInfo.CurrentCulture);
                ...
                var pageStringRectangle = new RectangleF((float) _leftMargin, _currentY,
            ...
            if (_isWithTitle)
            {
                ...
                var titleRectangle = new RectangleF((float) _leftMargin, _currentY,
                                                    (float) _pageWidth - (float) _rightMargin - (float) _leftMargin,
            ...
            if (_isCenterOnPage)
                currentX += (((float) _pageWidth - (float) _rightMargin - (float) _leftMargin) -
            ...
            var headerFont = _theDataGridView.ColumnHeadersDefaultCellStyle.Font;
            if (headerFont == null)
                // If there is no special HeaderFont style, then use the default DataGridView font style
                headerFont = _theDataGridView.DefaultCellStyle.Font;
                 ...
                 i++)
            {
                ...
                if (_theDataGridView.RowHeadersBorderStyle != DataGridViewHeaderBorderStyle.None)
        ...
        private bool DrawRows(Graphics g)
        {
            ...
            {
                if (_theDataGridView.Rows[_currentRow].Visible)
                    // Print the cells of the CurrentRow only if that row is visible
                {
                    ...
                    if (rowFont == null)
                        // If the there is no special font style of the CurrentRow, then use the default one associated with the DataGridView control
                        rowFont = _theDataGridView.DefaultCellStyle.Font;
                         ...
                         currentCell++)
                    {
                        ...
                        if (_theDataGridView.Columns[currentCell].DefaultCellStyle.Alignment.ToString().Contains("Right"))
                            ...
                            _theDataGridView.Columns[currentCell].DefaultCellStyle.Alignment.ToString()
                                                                .Contains("Center"))
                            cellFormat.Alignment = StringAlignment.Center;
                        else
                            cellFormat.Alignment = StringAlignment.Near;
                        ...
                        // Printing the cell text
                        g.DrawString(
                            _theDataGridView.Rows[_currentRow].Cells[currentCell].EditedFormattedValue.ToString(), rowFont,
                            rowForeBrush, cellBounds, cellFormat);

                        // Drawing the cell bounds
                        if (_theDataGridView.CellBorderStyle != DataGridViewCellBorderStyle.None)
        ...
        private static PrintDocument _printer;
        public static void PrintTable(DataTable table)
        {

            var gridview = new DataGridView { DataSource = table };
            _printer = new PrintDocument();
            

            _printer.PrintPage += (s, e) =>
                {
                    var bm = new Bitmap(gridview.Width, gridview.Height);
                    gridview.DrawToBitmap(bm, new Rectangle(0,0,gridview.Width,gridview.Height));
            ...
            _dgvPrinter = new DataGridViewPrinter(gridview, _printer, true, true, title, font, color, true);
            ...
            //_printer.PrintPage += (s, e) =>
            //{ e.HasMorePages = _dgvPrinter.DrawDataGridView(e.Graphics); };


            //var dlg = new PageSetupDialog {Document = printer, PageSettings = {Landscape = false}};
            //dlg.ShowDialog();
            
            var preview = new PrintPreviewDialog
                {
                    Document = _printer,
                    Height = (int) (Screen.PrimaryScreen.Bounds.Height*.75),
                    Width = (int) (Screen.PrimaryScreen.Bounds.Width*.75),
