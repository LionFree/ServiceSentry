...
namespace Ionic.Zip
{
    public partial class ZipEntry
    {
        ...
        /// </param>
        /// <returns>
        ///     the <c>ZipEntry</c> read from the stream.
        /// </returns>
        ...
        internal static int FindExtraFieldSegment(byte[] extra, int offx, UInt16 targetHeaderId)
        {
            ...
            {
                var headerId = (UInt16) (extra[j++] + extra[j++]*256);
        ...
        internal int ProcessExtraField(Stream s, Int16 extraFieldLength)
        {
            ...
            if (extraFieldLength > 0)
            {
                ...
                {
                    var start = j;
                    var headerId = (UInt16) (buffer[j++] + buffer[j++]*256);
        ...
        private int ProcessExtraFieldZip64(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            ...
            if (dataSize > 28)
                throw new BadReadException(
                    String.Format("  Inconsistent size (0x{0:X4}) for ZIP64 extra field at position 0x{1:X16}",
                                  dataSize, posn));
            ...
            var slurp = new Func<Int64>(() =>
                {
                    if (remainingData < 8)
                        throw new BadReadException(
                            String.Format("  Missing data for ZIP64 extra field, position 0x{0:X16}", posn));
        ...
        private int ProcessExtraFieldInfoZipTimes(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            if (dataSize != 12 && dataSize != 8)
                throw new BadReadException(
                    String.Format("  Unexpected size (0x{0:X4}) for InfoZip v1 extra field at position 0x{1:X16}",
                                  dataSize, posn));
        ...
        private int ProcessExtraFieldUnixTimes(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            ...
            if (dataSize != 13 && dataSize != 9 && dataSize != 5)
                throw new BadReadException(
                    String.Format(
                        "  Unexpected size (0x{0:X4}) for Extended Timestamp extra field at position 0x{1:X16}",
                        dataSize, posn));
        ...
        private int ProcessExtraFieldWindowsTimes(byte[] buffer, int j, Int16 dataSize, long posn)
        {
            ...
            if (dataSize != 32)
                throw new BadReadException(
                    String.Format("  Unexpected size (0x{0:X4}) for NTFS times extra field at position 0x{1:X16}",
                                  dataSize, posn));

            j += 4; // reserved
            var timetag = (Int16) (buffer[j] + buffer[j + 1]*256);
            var addlsize = (Int16) (buffer[j + 2] + buffer[j + 3]*256);
            j += 4; // tag and size
