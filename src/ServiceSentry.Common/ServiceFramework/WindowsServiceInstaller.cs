// -----------------------------------------------------------------------
//  <copyright file="WindowsServiceInstaller.cs" company="Curtis Kaler">
...
namespace ServiceSentry.Common.ServiceFramework
{
    ...
    public abstract class WindowsServiceInstaller
    {
        ...
        public static WindowsServiceInstaller GetInstance<T>() where T : WindowsService
        {
            ...
            if (info == null)
            {
                throw new ArgumentException(CommonStrings.EXCEPTION_ServiceMustBeAbstract);
            }

            var result = (T) info.Invoke(null, null);
            if (result == null)
            {
                throw new InvalidOperationException(String.Format(CommonStrings.EXCEPTION_CouldNotGetInstance, type.Name));
            }
        ...
        private sealed class WindowsServiceInstallerImplementation<T> : WindowsServiceInstaller where T : WindowsService
        {
            ...
            public WindowsServiceInstallerImplementation(T service)
            {
                ...
                if (attribute == null)
                {
                    throw new ArgumentException(CommonStrings.EXCEPTION_ServiceMustBeMarkedWithAttribute,
                                                "windowsServiceType");
                }
